...
def OurModel(input_shape, action_space, dueling):
    ...
    if dueling: 
        ...
        state_value = Lambda(lambda s: ops.tile(s, (1, int(action_space))))(state_value)

        action_advantage = Dense(int(action_space), kernel_initializer='he_uniform')(X)
        action_advantage = Lambda(lambda a: a - ops.mean(a, axis=1, keepdims=True))(action_advantage)
    ...
    model = Model(inputs = X_input, outputs = X, name = 'Cartpole_D3QN')
...
class DQNAgent:
    def __init__(self, env_name):
        ...
        self.max_steps = 4000
        self.env = gym.make(env_name, max_episode_steps=self.max_steps)
    ...
    def run(self):
        for e in range(self.EPISODES):
            ...
            while not done:
                ...
                if not done or i == self.max_steps - 1:
                    reward = reward
                else:
                    reward = -100
    ...
    def test(self):
        if not os.path.exists('models/' + self.Model_name):
            ...
            print("Run training first by uncommenting `agent.run()` in __main__.")
            return
        self.load('models/' + self.Model_name)
        for e in range(self.EPISODES):
            state, _ = self.env.reset()
            state = np.reshape(state, [1, self.state_size])
            done = False
